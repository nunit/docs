...
Optional element that may appear on tests or suites that were not executed. The element may also appear on tests that were short circuited using `Assert.Pass`. Contains a message giving the reason for skipping or short circuting the test.
