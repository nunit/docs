---
uid: attribute-datapointsource
---

# DatapointSource

`DatapointSourceAttribute` is used to mark a field, property, or method as providing a collection of data values for [Theory](xref:attribute-theory) tests. The source must return either an array of the required type or an `IEnumerable<T>`.
